...
如果你更倾向于用 Docker 运行 InkOS，本仓库提供开箱即用的 `Dockerfile` 和 `docker-compose.yml`。

**方式一：docker compose（推荐）**

```bash
git clone https://github.com/SeanAshe/inkos_docker.git
cd inkos_docker
...
docker run -d \
  --name inkos \
  -p 4567:4567 \
  -v inkos-workspace:/workspace \
  ghcr.io/seanashe/inkos:latest
```
...
docker run -d --name inkos -p 4567:4567 -v inkos-workspace:/workspace inkos
```

启动后访问 `http://localhost:4567` 即可使用 Studio Web 工作台。

**数据持久化**：`/workspace` 映射到命名卷 `inkos-workspace`，书籍、配置和产物都会保存在这里。

**环境变量**

| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `STUDIO_PORT` | `4567` | 宿主机映射的 Studio 端口 |
| `INKOS_VERSION` | `latest` | 构建镜像时安装的 `@actalk/inkos` 版本 |
| `INKOS_LLM_PROVIDER` | — | LLM 提供商（`custom` 等） |
| `INKOS_LLM_BASE_URL` | — | LLM API 地址 |
| `INKOS_LLM_API_KEY` | — | LLM API Key |
| `INKOS_LLM_MODEL` | — | 模型名称 |
| `INKOS_LLM_SERVICE` | — | 服务标识（`moonshot`、`google` 等） |

镜像内置 `HEALTHCHECK`，可通过 `docker inspect --format='{{.State.Health.Status}}' inkos` 查看健康状态。
